...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', opt=None):
        ...
        model = [nn.ReflectionPad2d(opt.first_kernel // 2),
                 nn.Conv2d(input_nc, ngf, kernel_size=opt.first_kernel, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]

        for i in range(opt.n_downsampling):  # add downsampling layers
            mult = 2 ** i
        ...
        for i in range(opt.n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                      ...
                      nn.ReLU(True)]
        model += [nn.ReflectionPad2d(opt.first_kernel // 2)]
        model += [nn.Conv2d(ngf, output_nc, kernel_size=opt.first_kernel, padding=0)]
        model += [nn.Tanh()]
